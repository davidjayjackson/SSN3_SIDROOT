...
def get_file_paths(
    ...
    if INTERACTIVE:
        ...
        try:
            ...
            raise ValueError(f"Invalid date entry: {exc}") from exc
    ...
    # Diagnostic — always printed so paths are visible in the rendered output
    print(f"  base_dir    : {base_dir}")
    print(f"  period_dir  : {period_dir}")
    print(f"  listings_dir: {listings_dir}  [{'EXISTS' if listings_dir.is_dir() else 'NOT FOUND'}]")
    print(f"  output_dir  : {output_dir}")

    # Validate source directory
    if not listings_dir.is_dir():
        raise FileNotFoundError(
            f"Event listings directory not found:\n  {listings_dir}\n"
            "Check that BASE_DIR, MONTH, and YEAR are correct."
        )
...
def get_event_qualifiers(input_func=None) -> list[str]:
    ...
    if not codes:
        raise ValueError(
            ...
            "Please specify FLA, XRA, or both in QUAL_CODES."
        )
