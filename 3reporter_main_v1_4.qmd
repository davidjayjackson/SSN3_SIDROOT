...
def resolve_run_paths(month: str, year: str, base_dir: str) -> dict:
    ...
    └── MMM_YY/                    ← date_dir  (e.g. JUN_11)
        ├── JUN_11XRA.txt           ← GOES XRA file (looked up from date_dir)
        ├── JUN_11FLA.txt           ← GOES FLA file (looked up from date_dir)
        ├── Data/                   ← observer .dat files go here
        │   └── *.dat
        └── Reports/                ← output files written here (created automatically)
    ...
    dict with keys:
        ...
        data_received <base>/JUN_11/Reports
    """
    mmm = month.upper()
    yy  = year.strip()
    ...
    data_recv = os.path.join(date_dir, "Reports")
...
XRA and FLA files are read from `control["date_dir"]` (the `MMM_YY/` folder),
keeping them separate from the output written to `MMM_YY/Reports/`.
...
print(f"  Date folder    : {paths['date_dir']}")
...
├── SIDAnalObservers.ini
├── SIDAnalStations.ini
└── JUN_11\                                ← created from PARAM_MONTH + PARAM_YEAR
    ├── JUN_11XRA.txt                      ← GOES XRA file (from 1goesevents.py)
    ├── JUN_11FLA.txt                      ← GOES FLA file (from 1goesevents.py)
    ├── Data\                              ← observer .dat files go here
    │   ├── A050NAA.dat
    │   └── A063NAA.dat
    └── Reports\                           ← created automatically; output written here
```
...
3. If using GOES correlation, place the `XRA.txt` and `FLA.txt` files
   (produced by `1goesevents.py`) in the `MMM_YY/` folder, or
...
Output files are written to `<PARAM_BASE_DIR>\<MMM_YY>\Reports\`.
